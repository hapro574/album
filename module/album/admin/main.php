...
if ($nv_Request->isset_request("action", "post,get")) {
    ...
    if ($id > 0 and $checksess == md5($id . NV_CHECK_SESSION)) {
        $db->query("DELETE FROM `nv4_vi_albuminfo` WHERE id=" . $id);
    }
}

if ($nv_Request->isset_request("change_status", "post,get")) {
    $id = $nv_Request->get_int('id', "post, get", 0);
    $sql = "SELECT id, status FROM nv4_vi_albuminfo WHERE id = " . $id;
    $result = $db->query($sql);
    if ($row = $result->fetch()) {
        $status = $row['status'] == 1 ? 0 : 1;
        $exe = $db->query("UPDATE `nv4_vi_albuminfo` SET status =" . $status . " WHERE id=" . $id);
        if ($exe) {
            die("OK");
        }
    }
...
if ($nv_Request->isset_request("change_weight", "post,get")) {
    ...
    if ($id > 0 and $new_weight > 0) {
        $sql = "SELECT id, weight FROM nv4_vi_albuminfo WHERE id !=  " . $id;
        $result = $db->query($sql);
        $weight = 0;
        while ($row = $result->fetch()) {
            ...
            $exe = $db->query("UPDATE `nv4_vi_albuminfo` SET weight =" . ($weight) . " WHERE id=" . $row['id']);
        }
        $exe = $db->query("UPDATE `nv4_vi_albuminfo` SET weight =" . $new_weight . " WHERE id=" . $id);
        if ($exe) {
            die('OK');
        }
    }
...
if (!empty($array_row)) {
   $i = ($page - 1) * $perpage;
    ...
    . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=add_album&amp;id=' . $row['id'];
    $row['url_delete'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name
    . '&amp;' . NV_OP_VARIABLE . '=main&amp;id=' . $row['id'] . '&action=delete&checksess=' . md5($row['id'] . NV_CHECK_SESSION);
